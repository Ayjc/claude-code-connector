...
ccb claude claude codex --instance claude=2  # 显式指定 claude 实例数
...
| `--instance` | provider 实例数覆盖（支持 `codex`、`claude`） | `ccb codex claude --instance codex=2` |
| `-h` | 查看详细帮助信息 | `ccb -h` |
| `-v` | 查看当前版本和检测更新 | `ccb -v` |
...
- **`ask <provider> <message>`** - 统一请求命令（默认后台）
  ...
  - codex/claude 多实例可使用 `--instance N`（例如 `ask codex --instance 2 "..."` 或 `ask claude --instance 2 "..."`）
